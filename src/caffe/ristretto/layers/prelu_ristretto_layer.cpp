...
PReLURistrettoLayer<Dtype>::PReLURistrettoLayer(const LayerParameter& param)
      : PReLULayer<Dtype>(param), BaseRistrettoLayer<Dtype>() 
{
...
{
	//调用父类的前向运算
	PReLULayer<Dtype>::Forward_gpu(bottom,top);
	//对结果进行量化输出
	this->QuantizeLayerOutputs_cpu(top[0]->mutable_cpu_data(), top[0]->count());
}

template <typename Dtype>
void PReLURistrettoLayer<Dtype>::Forward_gpu(
